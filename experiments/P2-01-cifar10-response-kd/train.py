...
def distill_train(config, teacher_model, train_loader, test_loader, device):
    print("\n" + "=" * 60)
    print("蒸馏训练学生模型 (ResNet-20 ← ResNet-56)")
    print("=" * 60)
    ...
    os.makedirs(config.checkpoint_dir, exist_ok=True)
    torch.save(student_model.state_dict(), os.path.join(config.checkpoint_dir, "student_distilled_resnet20.pth"))
    print(f"\n蒸馏学生模型最佳准确率: {best_acc:.2f}%")
