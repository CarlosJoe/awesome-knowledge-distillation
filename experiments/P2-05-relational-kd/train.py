...
def distill_train_rkd(config, teacher_model, train_loader, test_loader, device):
    print("\n" + "=" * 60)
    print("RKD 蒸馏训练学生模型 (ResNet-20 ← ResNet-56)")
    print("=" * 60)
    print(f"温度 T={config.temperature}, α={config.alpha}, w_dist={config.w_dist}, w_angle={config.w_angle}")
    ...
    for epoch in range(1, config.epochs + 1):
        ...
        for images, labels in train_loader:
            ...
            loss, kd_loss, ce_loss, dist_loss, angle_loss = rkd_loss(
            ...
            running_dist += dist_loss.item() * images.size(0)
            running_angle += angle_loss.item() * images.size(0)
        ...
        avg_dist = running_dist / total
        avg_angle = running_angle / total

        if epoch % 10 == 0 or epoch == config.epochs:
            test_acc = evaluate(student_model, test_loader, device)
            print(f"Epoch [{epoch}/{config.epochs}] 损失: {avg_loss:.4f} (KD: {avg_kd:.4f}, CE: {avg_ce:.4f}, Dist: {avg_dist:.4f}, Angle: {avg_angle:.4f}) 训练准确率: {train_acc:.2f}% 测试准确率: {test_acc:.2f}%")
            if test_acc > best_acc:
                best_acc = test_acc

    os.makedirs(config.checkpoint_dir, exist_ok=True)
    torch.save(student_model.state_dict(), os.path.join(config.checkpoint_dir, "student_rkd_resnet20.pth"))
    print(f"\nRKD蒸馏学生模型最佳准确率: {best_acc:.2f}%")
    config.rkd_acc = best_acc
    return student_model


def distill_train_response_kd(config, teacher_model, train_loader, test_loader, device):
    print("\n" + "=" * 60)
    print("纯 Response KD 蒸馏训练学生模型 (ResNet-20 ← ResNet-56)")
    print("=" * 60)
    ...
    for epoch in range(1, config.epochs + 1):
        ...
        for images, labels in train_loader:
            ...
            kd_loss = kl_loss_fn(soft_student, soft_teacher) * (config.temperature ** 2)
    ...
    os.makedirs(config.checkpoint_dir, exist_ok=True)
    torch.save(student_model.state_dict(), os.path.join(config.checkpoint_dir, "student_response_kd_resnet20.pth"))
    print(f"\nResponse KD蒸馏学生模型最佳准确率: {best_acc:.2f}%")
    config.response_kd_acc = best_acc
    return student_model
