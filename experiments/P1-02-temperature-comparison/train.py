...
def distill_with_temperature(teacher, train_loader, test_loader, device, T, config=Config()):
    set_seed(config.seed)
    student = StudentNet().to(device)
    criterion = DistillationLoss(T=T, alpha=config.alpha)
    ...
    for epoch in range(config.student_epochs):
        ...
        print(f'  [T={T}] Epoch {epoch+1}/{config.student_epochs}, Loss: {total_loss/len(train_loader):.4f}, Acc: {acc:.2f}%')

    final_acc = evaluate(student, test_loader, device)
    return student, final_acc, epoch_accs
...
def run_temperature_sweep(teacher, train_loader, test_loader, device, config=Config()):
    results = {}
    for T in config.T_values:
        ...
        student, final_acc, epoch_accs = distill_with_temperature(
            teacher, train_loader, test_loader, device, T, config
        )
        results[T] = {
            'student': student,
            'final_acc': final_acc,
